...
def get_card_description(query: str) -> dict:
    ...
    # 7. Qwen 4B 모델 프롬프트 (가독성 극대화)
    
    # 질문에서 "자세히", "상세히", "자세하게" 키워드 감지
    is_detailed = any(word in query for word in ["자세히", "상세히", "자세하게", "상세하게", "디테일하게"])
    
    template = """당신은 우리카드 상담 AI입니다. 아래 형식을 **정확히** 따라 답변하세요.

# 절대 규칙
- 카드 정보에 없는 내용은 절대 지어내지 마세요
- 숫자(연회비, 할인율, 날짜)는 카드 정보 그대로 사용
- 섹션 사이는 **빈 줄 1줄**, 항목 내부는 빈 줄 없음
- 줄글 금지! 반드시 • 불릿 포인트 사용
...
## 1. 단순 정보 질문 ("연회비?", "실적?", "출시일?")
→ **1-2문장 직답**

연회비는 12,000원이에요. (해외겸용 기준)
